...
class ReviewService:
    ...
    async def get_review_comment(self, comment_id: int) -> Optional[ReviewComment]:
    ...
    async def get_line_comments(
        ...
        return await self.review_comment_repo.get_line_comments(
            commit_id, file_path, line_number
        )
    ...
    async def create_commit_review(
        ...
        commit_id: int,
        reviewer_id: int,
        status: str = CommitReview.ReviewStatus.DRAFT
    ) -> CommitReview:
        ...
        existing = await self.commit_review_repo.get_by_commit_and_reviewer(
            commit_id, reviewer_id
        )
        if existing:
            raise ValueError(
    ...
    async def get_commit_review(self, review_id: int) -> Optional[CommitReview]:
    ...
    async def submit_for_review(
        ...
        return await self._update_review_status(
            commit_id,
            reviewer_id,
            CommitReview.ReviewStatus.PENDING
        )
    ...
    async def _update_review_status(
        ...
        if not existing:
            raise ValueError(
                f"No review found for commit {commit_id} by reviewer {reviewer_id}"
            )
    ...
    async def get_commits_needing_review(
        ...
        return await self.commit_review_repo.get_commits_needing_review(
            repository_id, limit
        )
    ...
    async def get_review_workload(self, reviewer_id: int) -> dict:
        ...
        completed = await self.commit_review_repo.get_completed_reviews(
            reviewer_id, limit=10
        )
