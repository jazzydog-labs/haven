...
class CommitService:
    ...
    async def create_commit(self, commit: Commit) -> Commit:
        ...
        if existing:
            raise ValueError(f"Commit {commit.commit_hash} already exists in repository {commit.repository_id}")
        
        return await self.commit_repository.create(commit)

    async def get_commit_by_id(self, commit_id: int) -> Optional[Commit]:
        """Get a commit by ID."""
        return await self.commit_repository.get_by_id(commit_id)

    async def get_commit_by_hash(self, repository_id: int, commit_hash: str) -> Optional[Commit]:
        """Get a commit by repository and hash."""
        return await self.commit_repository.get_by_hash(repository_id, commit_hash)

    async def get_commits_by_repository(self, repository_id: int, limit: int = 100, offset: int = 0) -> List[Commit]:
    ...
    async def sync_commits_from_git(self, repository_id: int, git_commits: List[dict]) -> List[Commit]:
    ...
    async def get_commit_review_by_id(self, review_id: int) -> Optional[CommitReview]:
    ...
    async def get_reviews_by_reviewer(self, reviewer_id: int, limit: int = 100, offset: int = 0) -> List[CommitReview]:
    ...
    async def get_commit_with_reviews(self, commit_id: int) -> Optional[dict]:
