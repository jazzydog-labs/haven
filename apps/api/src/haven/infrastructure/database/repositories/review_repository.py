...
from sqlalchemy import and_, func, select, delete, distinct
...
from haven.infrastructure.database.models import (
    ReviewCommentModel, 
    CommitReviewModel
)
...
class SqlAlchemyReviewCommentRepository(ReviewCommentRepository):
    ...
    async def get_by_commit_id(self, commit_id: int) -> List[ReviewComment]:
        ...
        stmt = (
            ...
            .order_by(ReviewCommentModel.file_path, ReviewCommentModel.line_number, ReviewCommentModel.created_at)
        )
        result = await self.session.execute(stmt)
...
class SqlAlchemyCommitReviewRepository(CommitReviewRepository):
    ...
    async def get_commits_needing_review(
        ...
        stmt = (
            ...
            .where(CommitModel.id.not_in(reviewed_commits_subquery))
        )
