...
class ReviewComment(BaseModel):
    ...
    line_number: Optional[int] = Field(None, description="Line number for line-specific comments")
    file_path: Optional[str] = Field(None, max_length=500, description="File path for file-specific comments")
    ...
    def update_status(self, new_status: str) -> 'CommitReview':
        ...
        return self.__class__(
            **{
                **self.dict(),
                'status': new_status,
                'reviewed_at': datetime.now(timezone.utc) if new_status in [
                    self.ReviewStatus.APPROVED, 
                    self.ReviewStatus.NEEDS_REVISION
