...
class TestReviewComment:
    ...
    def test_create_review_comment_minimal(self):
        ...
        comment = ReviewComment(commit_id=1, reviewer_id=1, content="This looks good!")
    ...
    def test_create_general_comment(self):
        ...
        comment = ReviewComment(
            commit_id=1, reviewer_id=1, content="Overall this commit looks good."
        )
    ...
    def test_validate_line_number_positive(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            ReviewComment(commit_id=1, reviewer_id=1, line_number=0, content="Test comment")
        assert "Line number must be positive" in str(exc_info.value)

        with pytest.raises(ValidationError) as exc_info:
            ReviewComment(commit_id=1, reviewer_id=1, line_number=-5, content="Test comment")
        assert "Line number must be positive" in str(exc_info.value)
    ...
    def test_validate_file_path_format(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            ReviewComment(
                commit_id=1, reviewer_id=1, file_path="/absolute/path.py", content="Test comment"
            )
        ...
        with pytest.raises(ValidationError) as exc_info:
            ReviewComment(
                commit_id=1, reviewer_id=1, file_path="../parent/file.py", content="Test comment"
            )
    ...
    def test_validate_content_not_empty(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            ReviewComment(commit_id=1, reviewer_id=1, content="")
    ...
    def test_content_whitespace_trimmed(self):
        ...
        comment = ReviewComment(commit_id=1, reviewer_id=1, content="  Good work!  \n")
        assert comment.content == "Good work!"

    def test_update_content(self):
        """Test updating comment content."""
        original = ReviewComment(commit_id=1, reviewer_id=1, content="Original content")
...
class TestCommitReview:
    ...
    def test_create_commit_review(self):
        ...
        review = CommitReview(commit_id=1, reviewer_id=1, status=CommitReview.ReviewStatus.PENDING)
    ...
    def test_validate_status(self):
        ...
        for status in CommitReview.ReviewStatus.all_values():
            review = CommitReview(commit_id=1, reviewer_id=1, status=status)
            assert review.status == status

        # Invalid status should fail
        with pytest.raises(ValidationError) as exc_info:
            CommitReview(commit_id=1, reviewer_id=1, status="invalid_status")
    ...
    def test_review_status_properties(self):
        ...
        draft_review = CommitReview(
            commit_id=1, reviewer_id=1, status=CommitReview.ReviewStatus.DRAFT
        )
        ...
        pending_review = CommitReview(
            commit_id=1, reviewer_id=1, status=CommitReview.ReviewStatus.PENDING
        )
        ...
        approved_review = CommitReview(
            commit_id=1, reviewer_id=1, status=CommitReview.ReviewStatus.APPROVED
        )
        ...
        needs_revision_review = CommitReview(
            commit_id=1, reviewer_id=1, status=CommitReview.ReviewStatus.NEEDS_REVISION
        )
    ...
    def test_update_status(self):
        ...
        original = CommitReview(commit_id=1, reviewer_id=1, status=CommitReview.ReviewStatus.DRAFT)
