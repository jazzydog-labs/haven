...
export const recordsAPI = {
  list: async (limit: number = 10, offset: number = 0): Promise<RecordListResponse> => {
    const response = await fetch(`${API_BASE_URL}/api/v1/records?limit=${limit}&offset=${offset}`, {
      headers: {
  ...
  partialUpdate: async (id: string, data: RecordUpdateDTO): Promise<RecordItem> => {
    ...
    if (!response.ok) {
      throw new Error(`Failed to partially update record: ${response.statusText}`);
    }
